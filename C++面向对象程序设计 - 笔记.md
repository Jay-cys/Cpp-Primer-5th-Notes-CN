...
- 友元函数可以自由取得private成员，不声明为friend的话，也可以通过函数来取得私有变量
  ...
  <img src="https://gitee.com/sparkle_zz/markdown-pics/raw/master/image-20201107193914915.png" alt="image-20201107193914915" style="zoom: ;" />

- 相同类的各个实例对象互为友元，可以通过彼此的内部方法调用传入参数的内部私有成员变量。

  <img src="https://i.imgur.com/72ZMZHz.png" style="zoom:;" />
...
<img src="https://gitee.com/sparkle_zz/markdown-pics/raw/master/image-20201107220011796.png" alt="image-20201107220011796" style="zoom:80;" />
...
<img src="https://gitee.com/sparkle_zz/markdown-pics/raw/master/image-20201107222453706.png" alt="image-20201107222453706" style="zoom:80%;" />

- 如果没有自我赋值检测，那么自身对象的m_data将被释放，m_data指向的内容将不存在，所以该拷贝会出问题。

![拷贝赋值](E:\CYJ\Learning\Cpp-Primer-5th-Notes-CN\image\拷贝赋值.png)
...
- new 的过程：先分配memory，再调用ctor构造函数
  ...
  - 转型，将`void*`转为`Complex*`
  - 调用构造函数

<img src="https://i.imgur.com/CoqboeP.png" style="zoom: 80%;" />
...
- 非静态成员函数，调用成员函数时，其实会传入一个调用者的地址 `c1.real() -> c1.real(&c1)`，返回调用者的实部real，即this->real，谁调用谁是real
  ...
  <img src="https://gitee.com/sparkle_zz/markdown-pics/raw/master/image-20201109142243499.png" alt="image-20201109142243499" style="zoom:80%;" />
...
- ### **单例模式**- 构造函数放在private中，不允许外界构造对象，但可以通过public函数取得private成员，加上关键字static，该对象只能有一个
  ...
  <img src="https://gitee.com/sparkle_zz/markdown-pics/raw/master/image-20201109143344277.png" alt="image-20201109143344277" style="zoom:80%;" />

  ### Note
  
  静态函数通常用来操作静态数据，这是环环相扣的。
  
  静态函数与普通成员函数的区别：
  
  -   普通成员函数首先需要构造类对象，然后调用类成员函数；传递地址
  -   静态函数可以直接通过类名来调用；无需传递地址

## 16. 模板 template

**类模板 class template**

<img src="https://gitee.com/sparkle_zz/markdown-pics/raw/master/image-20201109143917117.png" alt="image-20201109143917117" style="zoom: 80%;" />

**函数模板 function template**

<img src="https://gitee.com/sparkle_zz/markdown-pics/raw/master/image-20201109145545362.png" alt="image-20201109145545362" style="zoom:80%;" />

### Note

`complex<double>`中`double`会代替所有的`T`

`template <calss T>`与 `template <typename T>`中的`class`和`typename`是一样的，只是`typename`是c++98后才引入的

## 17. namespace 命名空间

<img src="https://gitee.com/sparkle_zz/markdown-pics/raw/master/image-20201109150733964.png" alt="image-20201109150733964" style="zoom: 80;" />
...
Fraction类对象转换为 double 类型，自动调用 double() 函数转换：
...
- 成员模板在泛型编程里用得较多，为了有更好的可扩展性，以上图为例，T1往往是U1的基类，T2往往是U2的基类：
  ...
  <img src="https://gitee.com/sparkle_zz/markdown-pics/raw/master/image-20201110220153084.png" alt="image-20201110220153084" style="zoom:80%;" />
