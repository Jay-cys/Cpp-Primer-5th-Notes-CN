...
<img src=".assets/image-20230620165903239.png" alt="image-20230620165903239" style="zoom:67%;" />

`shared_ptr`独有的操作：

<img src=".assets/image-20230620165919026.png" alt="image-20230620165919026" style="zoom:67%;" />
...
// p5 points to an int that is value initialized，值为0
...
r = q;  // assign to r, making it point to a different address，给r赋值，令他指向另一个地址
        // increase the use count for the object to which q points，递增q指向的对象的引用计数
        // reduce the use count of the object to which r had pointed，递减r原来指向的对象的引用计数
        // the object r had pointed to has no users; that object is automatically freed，r若原来指向的对象已没有引用者，会自动释放
```
...
>   p1的类型是一个指针，指向从obj自动推断出的类型，若obj是一个int，那么p1就是int*
...
```c++
shared_ptr<int> p1 = new int(1024);    // error: must use direct initialization，不是直接初始化形式
shared_ptr<int> p2(new int(1024));     // ok: uses direct initialization
```

默认情况下，用来初始化智能指针的内置指针必须指向动态内存，因为智能指针默认使用`delete`释放它所管理的对象。如果要将智能指针绑定到一个指向其他类型资源的指针上，就必须提供自定义操作来代替`delete`。

<img src=".assets/image-20230620200652141.png" alt="image-20230620200652141" style="zoom: 67%;" />

<img src=".assets/image-20230620200708194.png" alt="image-20230620200708194" style="zoom:67%;" />

不要混合使用内置指针和智能指针，这也是为什么推荐使用`make_shared`而不是`new`的原因。当将`shared_ptr`绑定到内置指针后，资源管理就应该交由`shared_ptr`负责。不应该再使用内置指针访问`shared_ptr`指向的内存。

>   以下函数process中，虽然不能给它传递一个内置指针，但可以给它传递一个由内置指针显示构造的智能指针。最好是给它直接传递一个智能指针
...
shared_ptr<int> p(new int(42));   // reference count is 1,引用计数为1
process(p);     // copying p increments its count; in process the reference count is 2，拷贝p会递增它的引用计数，在process中引用计数指为2
int i = *p;     // ok: reference count is 1，正确，引用计数指为1
```

智能指针的`get`函数返回一个内置指针，指向智能指针管理的对象。主要用于向不能使用智能指针的代码传递内置指针。使用`get`返回指针的代码不能`delete`此指针（如果delete，那么原本的智能指针就成了野指针（指针所指的对象被删除））。==不要使用`get`初始化另一个智能指针或为智能指针赋值。==
...
```c++
if (!p.unique())
    p.reset(new string(*p));   // we aren't alone; allocate a new copy，我们不是唯一用户，分配新的拷贝
*p += newVal;   // now that we know we're the only pointer, okay to change this object
```

### 智能指针和异常（Smart Pointers and Exceptions）

如果使用智能指针，即使程序块过早结束（异常结束），智能指针类也能确保在内存不再需要时将其释放。

>   函数的退出有两种可能，正常结束和异常结束，无论那种情况，局部对象都会被销毁。在下面的程序中，
>
>   第一种是通过内置指针来管理new动态分配的内存，若在delete之前发生异常，那么该指针会被销毁，但是指针所指向的内存不会被释放。
>
>   第二种是通过智能指针来管理动态分配的内存，若抛出异常，由于共享指针销毁时会检查引用计数，而sp是指向这块内存的唯一指针，因此内存会被成功释放。
...
>   下面代码中，若在函数`f( )`中没有智能指针p，那么在函数结束时，connection无法正常关闭。
>
>   而有这个智能指针p，那么在函数结束时，会销毁智能指针，且会调用end_connection来关闭connection
...
- ==不释放(delete)`get`返回的指针。==

- ==不使用`get`初始化或`reset`另一个智能指针。==

- ==使用`get`返回的内置指针时，如果最后一个对应的智能指针被销毁，该内置指针就无效了。==
...
<img src=".assets/image-20230620205344610.png" alt="image-20230620205344610" style="zoom:67%;" />
...
p2.reset(p3.release()); // reset deletes the memory to which p2 had pointed，reset释放了p2原来指向的内存
```

调用`release`会切断`unique_ptr`和它原来管理的对象之间的联系。==`release`返回的指针通常被用来初始化另一个智能指针或给智能指针赋值。==如果没有用另一个智能指针保存`release`返回的指针，程序就要负责资源的释放。

```c++
p2.release();   // WRONG: p2 won't free the memory and we've lost the pointer，错误，p2不会释放内存，而且也会丢失指针
auto p = p2.release();   // ok, but we must remember to delete(p)
```

不能拷贝`unique_ptr`的规则有一个例外：可以拷贝或赋值一个即将被销毁的`unique_ptr`（移动构造、移动赋值）。

>   下面代码，返回了一个局部对象，该局部对象是即将被销毁的
...
类似`shared_ptr`，`unique_ptr`默认情况下用`delete`释放其指向的对象。`unique_ptr`的删除器同样可以重载，但`unique_ptr`管理删除器的方式与`shared_ptr`不同。定义`unique_ptr`时必须在尖括号中提供删除器类型。创建或`reset`这种`unique_ptr`类型的对象时，必须提供一个指定类型的可调用对象（删除器）。
...
<img src=".assets/image-20230620210426208.png" alt="image-20230620210426208" style="zoom:67%;" />
...
动态数组不同于数组，动态数组是new出来的，位于堆空间，而数组是直接初始化的，位于栈空间。
...
up.release();   // automatically uses delete[] to destroy its pointer，自动用delete[]销毁其指针
```

指向数组的`unique_ptr`：

<img src=".assets/image-20230620212456721.png" alt="image-20230620212456721" style="zoom:67%;" />
...
<img src=".assets/image-20230620214844317.png" alt="image-20230620214844317" style="zoom:67%;" />
...
<img src=".assets/image-20230620215418060.png" alt="image-20230620215418060" style="zoom:67%;" />
